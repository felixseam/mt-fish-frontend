...
export function createFishRendererFactory(options: {
  ...
  function createAnimatedFishBySpawnFishId(
    ...
    if (!state) {
      ...
      // Stop pixi-spine from registering its own listener on
      // PIXI.Ticker.shared. Each Spine instance normally drives itself
      // independently every frame. We disable it here and drive it
      // manually from useFishContextMachine's single tick loop instead
      // (see updateFish()).
      ...
      // Same reasoning — the shadow is a second independent Spine
      // instance and would otherwise register its own ticker too.
