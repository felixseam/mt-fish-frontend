...
<template>
  ...
  <OceanCardDialogShell v-model="dialogVisible" max-width="520" :persistent="true" scrim-color="rgba(0,0,0,0.7)"
    card-class="profile-card-shell">
    <template #title-prefix>
      <v-btn v-if="activeView === 'password'" icon variant="text" size="small" @click="activeView = 'profile'">
    ...
    <template v-if="activeView === 'profile'">
      <v-card-text class="dialog-scroll-area ">
        ...
        <div class="action-list">
          <v-btn size="large" variant="outlined" color="grey-lighten-1" class="text-capitalize action-btn flex-1"
      ...
      <div class="dialog-scroll-area">
        <v-card-text class="pa-4 pa-sm-6">
          <v-form ref="passwordForm" v-model="passwordFormValid" @submit.prevent="submitPassword">
            <v-text-field v-model="oldPassword" :label="t('profile.currentPassword')"
              :type="showOldPassword ? 'text' : 'password'" :rules="[rules.required]"
              prepend-inner-icon="mdi-lock-outline" :append-inner-icon="showOldPassword ? 'mdi-eye-off' : 'mdi-eye'"
              @click:append-inner="showOldPassword = !showOldPassword" variant="outlined" color="cyan-lighten-2"
              bg-color="rgba(10, 40, 64, 0.9)" class="mb-2" autocomplete="current-password" />

            <v-text-field v-model="newPassword" :label="t('profile.newPassword')"
              :type="showNewPassword ? 'text' : 'password'" :rules="[rules.required, rules.minLength]"
              prepend-inner-icon="mdi-lock-plus-outline"
              :append-inner-icon="showNewPassword ? 'mdi-eye-off' : 'mdi-eye'"
              @click:append-inner="showNewPassword = !showNewPassword" variant="outlined" color="cyan-lighten-2"
              bg-color="rgba(10, 40, 64, 0.9)" class="mb-2" autocomplete="new-password" />
            ...
            <div v-if="newPassword" class="mb-4">
              ...
              <v-progress-linear :model-value="passwordStrengthValue" :color="passwordStrengthBarColor" rounded
                height="4" />
            </div>

            <v-text-field v-model="confirmPassword" :label="t('profile.confirmNewPassword')"
              :type="showConfirmPassword ? 'text' : 'password'" :rules="[rules.required, rules.matchPassword]"
              prepend-inner-icon="mdi-lock-check-outline"
              :append-inner-icon="showConfirmPassword ? 'mdi-eye-off' : 'mdi-eye'"
              @click:append-inner="showConfirmPassword = !showConfirmPassword" variant="outlined" color="cyan-lighten-2"
              bg-color="rgba(10, 40, 64, 0.9)" autocomplete="new-password" />

            <v-alert v-if="passwordError" type="error" variant="tonal" density="compact" closable class="mt-3"
        ...
        <v-card-actions class="px-4 px-sm-6 pb-4 pt-2 flex-shrink-0">
          ...
          <v-btn color="cyan-lighten-2" variant="flat" :disabled="!passwordFormValid" :loading="savingPassword"
            min-width="120" @click="submitPassword">
  ...
  <OceanCardDialogShell v-model="avatarDialogVisible" max-width="520" :persistent="true" scrim-color="rgba(0,0,0,0.7)"
    card-class="profile-card-shell">
    <template #title-prefix>
    ...
    <div class="dialog-scroll-area">
      <v-card-text class="pa-4 pa-sm-6">
        ...
        <div class="avatar-grid">
          <div v-for="(avatar, index) in avatarList" :key="index" class="avatar-slot"
            :class="{ 'avatar-selected': selectedAvatar === avatar.path }" @click="selectAvatar(avatar.path)">
            <v-avatar size="72" class="avatar-item" :class="{ 'selected-ring': selectedAvatar === avatar.path }">
      ...
      <v-card-actions class="px-4 px-sm-6 pb-4 pt-2 flex-shrink-0">
        ...
        <v-btn color="cyan-lighten-2" variant="flat" :disabled="selectedAvatar === currentAvatarSource"
          :loading="savingAvatar" min-width="120" class="text-capitalize" @click="confirmAvatar">
          {{ t('common.confirm') }}
        </v-btn>
      </v-card-actions>
    </div>
